...
-l: min continuous match length threshold [200]
-q: quiet, don't print anything

Examples:

- Default: Exact match only (assuming VR sequence length is 200bp)
$YW$0$N -i ${LCY}example.fastq.gz$N -b ${LGN}example_VR.fa$N -l ${LPR}200$N

- At least 100bp of VR sequence match:
$YW$0$N -i ${LCY}example.fastq.gz$N -b ${LGN}example_VR.fa$N -l ${LPR}100$N
...
It'll stop when finding a match or when window size is less than min length threshold [-l, default 200]
...
my $minlen_threshold_orig = defined $opt_l ? $opt_l : 0;
...
my $logFile = "$outDir/$fileName1\_$VRFilename\_l$minlen_threshold_orig\_w$window_subtract_size\_s$step_size.log";
my $outFile = "$outDir/$fileName1\_$VRFilename\_l$minlen_threshold_orig\_w$window_subtract_size\_s$step_size.tsv";
my $outFileshort = "$outDir/$fileName1\_$VRFilename\_l$minlen_threshold_orig\_w$window_subtract_size\_s$step_size.short.tsv";
my $VRstatsFile = "$outDir/$fileName1\_$VRFilename\_l$minlen_threshold_orig\_w$window_subtract_size\_s$step_size.stats";
